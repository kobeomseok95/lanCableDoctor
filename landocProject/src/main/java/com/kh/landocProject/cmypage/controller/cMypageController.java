...
public class cMypageController {
	...
	public void orderDetail(HttpSession session, HttpServletResponse response, OrderList order,
			@RequestParam(value = "orderNo") int orderNo) throws JsonIOException, IOException {
		Client loginClient = (Client) session.getAttribute("loginClient");
		String cNo = loginClient.getcNo();
		order.setcNo(cNo);
	...
	public ModelAndView orderListDateSearch(HttpSession session, ModelAndView mv,
			@RequestParam(value = "date") String date, OrderList order) throws cMypageException {
		Client loginClient = (Client) session.getAttribute("loginClient");
		String cNo = loginClient.getcNo();
		order.setcNo(cNo);
		order.setDate(date);
	...
	public ModelAndView orderListDateSearch2(HttpSession session, ModelAndView mv,
