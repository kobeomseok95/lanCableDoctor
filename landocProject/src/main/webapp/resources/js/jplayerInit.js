...
(function($) {
	...
	.each(
			function() {
				// var ffaction;
				var rwaction, rewinding, fastforward, thisItem = $(this), player = thisItem
				.find('.jplayer');
				...
				function GetPlayerProgress() {
					return (thisItem.find('.jp-play-bar').width()
				...
				function RewindTrack() {
					...
					// If it goes past the starting point - stop
					// rewinding and pause
					if (futureProgress <= 0) {
						rewinding = false;
					...
					// Continue rewinding
						...
						player.jPlayer("playHead", parseInt(
								futureProgress, 10));
					}
				}
				...
				function FastforwardTrack() {
					...
					var currentProgress = GetPlayerProgress();
					var futureProgress = currentProgress + 5;
					// If the percentage exceeds the max - stop fast
					// forwarding at the
					// end.
					if (futureProgress >= 100) {
						fastforward = false;
						window.clearInterval(ffaction);
						player.jPlayer("playHead",
								parseInt($('.jp-duration').text()
										.replace(':', '')));
					} else {
						player.jPlayer("playHead", parseInt(
								futureProgress, 10));
